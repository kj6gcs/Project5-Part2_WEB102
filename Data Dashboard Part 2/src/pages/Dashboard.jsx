...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchLaunches = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch launch data. Status: ${response.status}`,
          );
        }
        ...
        setError(
          err.message || "Something went wrong while fetching launch data.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const providerChartData = useMemo(() => {
    return Object.entries(
      filteredLaunches.reduce((acc, launch) => {
  ...
  const launchesWithMission = launches.filter(
    ...
    >
      <div className="app">
        <header className="header">
          <h1>Orbital Watch+</h1>
          <p>
            Explore upcoming launches, filter results, and click any launch to
            view more detailed mission information.
          </p>
          <p>
            Use the dashboard controls to narrow by status, provider, or launch
            date.
          </p>
        </header>
        ...
        {showCharts && (
          <section className="charts-section">
            <h2>Launch Insights</h2>
            <p>
              These charts highlight which providers dominate the upcoming
              launch schedule and how launch readiness varies by current status.
            </p>

            <div className="chart-container">
              <h3>Launches by Provider</h3>
